...
class IntegratedRunEngine:
    ...
    async def execute_integrated_run(self, slug: str):
        ...
        try:
            ...
            integrated_run.current_stage = 'Starting simulations'
            session.commit()
    ...
    async def _execute_parallel_simulations(self, integrated_run: IntegratedRun,
        ...
        # Update progress
        integrated_run.current_stage = 'Running simulations'
        ...
        for sim_run in simulation_runs:
            task = self._execute_single_simulation(sim_run, session)
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"Simulation {simulation_runs[i].simulation_type} failed: {result}")
                simulation_runs[i].status = 'error'
            else:
                simulation_runs[i].status = 'completed'
                simulation_runs[i].completed_at = datetime.utcnow()
    ...
    async def _execute_single_simulation(self, sim_run: SimulationRun, session):
        ...
        # Execute simulation
        logger.info(f"Executing {sim_run.simulation_type} simulation: {' '.join(cmd)}")
        ...
        try:
            ...
            # Store results path - training.sh creates files with timestamps
            ...
            if results_path:
                ...
                try:
                    with open(results_path, 'r') as f:
                        ...
                        sim_run.generations = results_data.get('total_generations', params.get("generations", 50))
    ...
    async def _generate_visualizations(self, integrated_run: IntegratedRun,
        ...
        print(f"🎬 [VISUALIZATION] Starting visualization generation for {integrated_run.slug}")
        print(f"🎬 [VISUALIZATION] Number of simulation runs: {len(simulation_runs)}")

        integrated_run.current_stage = 'Generating visualizations'
        ...
        try:
            ...
            # Generate side-by-side animation
            print(f"🎬 [VISUALIZATION] Starting side-by-side animation generation...")
            await self._create_side_by_side_animation(simulation_runs, viz_dir)
            print(f"🎬 [VISUALIZATION] Side-by-side animation completed")

            # Generate frame comparisons
            print(f"🎬 [VISUALIZATION] Starting frame comparisons generation...")
            await self._create_frame_comparisons(simulation_runs, viz_dir)
            print(f"🎬 [VISUALIZATION] Frame comparisons completed")
            ...
            print(f"🎬 [VISUALIZATION] Files created: {[f.name for f in files_created]}")

            integrated_run.progress = 0.9
            session.commit()
            print(f"🎬 [VISUALIZATION] Visualization generation completed successfully")
    ...
    async def _create_side_by_side_animation(self, simulation_runs: List[SimulationRun], output_dir: Path):
        """Create side-by-side animation GIF"""

        logger.info("Creating side-by-side animation")
        print(f"🎬 [VISUALIZATION] Starting side-by-side animation creation")
        ...
        try:
            ...
            for frame_idx in range(frame_count):
                ...
                for i, sim_run in enumerate(simulation_runs):
                    # Simulate different patterns for each simulation type
