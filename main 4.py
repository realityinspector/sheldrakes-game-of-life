        ...
        <script>
                ...
                if (files.length > 0 || isComplete) {{
                    contentHtml += `
                        <div class="educational-walkthrough" style="background: white; padding: 20px; border: 2px solid #4CAF50; border-radius: 8px; margin: 20px 0;">
                            <h3 style="color: #4CAF50; font-size: 24px; margin-bottom: 15px;">🧬 Morphic Resonance Analysis Results</h3>
                ...
                if (isComplete) {{
                    contentHtml += `
                        <div style="background: #E8F5E8; padding: 20px; border-radius: 8px; margin: 20px 0;">
                            ...
                            <p>All visualizations and statistical analysis have been generated. The morphic resonance patterns show fascinating emergence behaviors across different simulation modes.</p>
                        </div>
                    `;
                }}
            ...
            async function checkFileGenerationProgress(status) {{
                try {{
                    ...
                    document.getElementById('current-stage').textContent =
                        `Generating files (${{fileData.files_generated}}/${{fileData.total_expected_files}})`;
            ...
            async function updateStatus() {{
                try {{
                    ...
                    // Check file generation progress if status shows completed
            ...
            if (initialStatus === 'completed') {{
                ...
                // Try to load results, fallback to basic completed state if API unavailable
                const handleCompletedRun = async () => {{
                    console.log('📋 handleCompletedRun() called');
                    try {{
                        // First update the status display, then load results
                        console.log('🔄 About to call updateStatus()...');
                        try {{
                            await updateStatus();
                            console.log('✅ updateStatus() completed');
                        }} catch (statusError) {{
                            console.log('⚠️  updateStatus() failed:', statusError);
                        }}
                        // loadResults() is already called by updateStatus() when status is completed
                        // so we don't need to call it again here to avoid race conditions
                        console.log('✅ Results already loaded by updateStatus() - skipping duplicate loadResults() call');
                    }} catch (error) {{
                        console.log('⚠️  API unavailable, showing basic completion message...', error);
                        document.getElementById('results-content').innerHTML = `
                            <div class="results-summary">
                                <h3>🎉 Analysis Complete!</h3>
                                <p>✅ Your morphic resonance analysis has completed successfully.</p>
                                <p>📁 All visualization assets have been generated.</p>
                                <p>🔄 Please restart the server to view detailed results.</p>
                                <div style="margin: 20px 0;">
                                    <strong>Available Files:</strong>
                                    <ul style="margin: 10px 0; padding-left: 20px;">
                                        <li>Side-by-side evolution animations</li>
                                        <li>Frame-by-frame comparisons</li>
                                        <li>Statistical analysis charts</li>
                                        <li>Morphic insights visualization</li>
                                    </ul>
                                </div>
                                <p><em>Restart the server with <code>./run.sh</code> to view the full interactive analysis.</em></p>
                            </div>
                        `;
                    }}
                }};
