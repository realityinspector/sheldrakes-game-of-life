    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p>Comprehensive morphic resonance analysis</p>
            </div>
        ...
        <script>
            ...
            async function updateStatus() {{
                try {{
                    ...
                    if (status.status === 'completed') {{
                        ...
                        // Multiple fallback refresh mechanisms
                        let refreshed = false;

                        // Primary refresh mechanism
                        console.log('Integrated run completed - refreshing in 3 seconds...');
                        setTimeout(() => {{
                            if (!refreshed) {{
                                refreshed = true;
                                console.log('Refreshing page to show full results...');
                                window.location.reload();
                            }}
                        }}, 3000);

                        // Backup refresh mechanism (in case setTimeout fails)
                        setTimeout(() => {{
                            if (!refreshed) {{
                                refreshed = true;
                        ...
                        // Immediate refresh if user clicks anywhere (fallback for interaction)
                        const immediateRefresh = () => {{
                            if (!refreshed) {{
                                refreshed = true;
                                console.log('User interaction triggered refresh...');
                                window.location.reload();
                            }}
                        }};

                        document.addEventListener('click', immediateRefresh, {{ once: true }});
                        document.addEventListener('keypress', immediateRefresh, {{ once: true }});
            ...
            async function loadResults() {{
                try {{
                        ...
                        <div class="results-summary">
                            ...
                            <div class="summary-grid">
                                <div class="summary-card">
                                    <h4>📊 Simulation Parameters</h4>
                                    <p><strong>Generations:</strong> ${{runData.parameters.generations}}</p>
                                    <p><strong>Grid Size:</strong> ${{runData.parameters.grid_size}}×${{runData.parameters.grid_size}}</p>
                                    <p><strong>Crystal Count:</strong> ${{runData.parameters.crystal_count}}</p>
                                    <p><strong>Initial Density:</strong> ${{runData.parameters.initial_density}}</p>
                                </div>
                                <div class="summary-card">
                    ...
                    runData.parameters.simulation_types.forEach(type => {{
                        const typeNames = {{
                            'morphic': '🧬 Morphic Resonance',
                            'llm_control': '🤖 LLM Control',
                            'classical': '⚛️ Classical Conway'
                        }};
                        resultsHtml += `<li>${{typeNames[type] || type}}</li>`;
                    }});
                    ...
                    resultsHtml += `
                                ...
                                <div class="summary-card">
                                    <h4>⏱️ Execution Details</h4>
                                    <p><strong>Created:</strong> ${{new Date(runData.created_at).toLocaleString()}}</p>
                                    <p><strong>Status:</strong> ${{runData.status.toUpperCase()}}</p>
                                    ${{runData.completed_at ? `<p><strong>Completed:</strong> ${{new Date(runData.completed_at).toLocaleString()}}</p>` : ''}}
                                </div>
                            </div>
                        </div>

                        <!-- Side-by-Side Animations Section -->
                        <div class="analysis-section">
                            <h3>📹 Side-by-Side Evolution Animations</h3>
                            <div class="animations-grid">
                    `;

                    if (analysis.animations) {{
                        Object.entries(analysis.animations).forEach(([simType, animData]) => {{
                            if (simType !== 'side_by_side') {{
                                resultsHtml += `
                                    <div class="animation-card">
                                        <h4>${{simType.charAt(0).toUpperCase() + simType.slice(1)}} Evolution</h4>
                                        <img src="/${{animData.path}}" alt="${{simType}} animation" class="evolution-animation" />
                                        <p>Frames: ${{animData.frames}} | Duration: ${{animData.duration_ms/1000}}s</p>
                                    </div>
                                `;
                            }}
                        }});

                        if (analysis.animations.side_by_side) {{
                            resultsHtml += `
                                <div class="side-by-side-animation">
                                    <h4>🔄 Side-by-Side Comparison</h4>
                                    <img src="/${{analysis.animations.side_by_side.path}}" alt="Side-by-side comparison" class="comparison-animation" />
                                    <p>${{analysis.animations.side_by_side.description}}</p>
                                </div>
                            `;
                        }}
                    }}
                    ...
                    resultsHtml += `
                        ...
                        <div class="analysis-section">
                            <h3>🖼️ Frame-by-Frame Analysis</h3>
                            <div class="frames-grid">
                    `;

                    if (analysis.frame_analysis && analysis.frame_analysis.frames) {{
                        analysis.frame_analysis.frames.forEach(frame => {{
                            resultsHtml += `
                                <div class="frame-card">
                                    <img src="/${{frame.visualization}}" alt="Generation ${{frame.generation}}" />
                                    <div class="frame-info">
                                        <h5>Generation ${{frame.generation}}</h5>
                                        <div class="frame-stats">
                            `;

                            Object.entries(frame).forEach(([simType, data]) => {{
                                if (simType !== 'visualization' && simType !== 'generation' && typeof data === 'object') {{
                                    resultsHtml += `
                                        <div class="sim-stats">
                                            <strong>${{simType}}:</strong> Pop ${{data.population}}, Density ${{(data.density*100).toFixed(1)}}%
                                        </div>
                                    `;
                                }}
                            }});
                    ...
                    if (analysis.statistical_analysis) {{
                        const stats = analysis.statistical_analysis;

                        // Add visualizations
                        if (stats.visualizations) {{
                            Object.entries(stats.visualizations).forEach(([key, path]) => {{
                                resultsHtml += `
                                    <div class="chart-container">
                                        <img src="/${{path}}" alt="${{key.replace('_', ' ')}}" class="analysis-chart" />
                                    </div>
                                `;
                            }});
                        }}
                        ...
                        if (stats.correlations) {{
                            ...
                            `;
                            Object.entries(stats.correlations).forEach(([key, value]) => {{
                                resultsHtml += `
                                    <div class="correlation-item">
                                        <span>${{key.replace('_', ' ')}}</span>
                                        <span class="correlation-value">${{value.toFixed(3)}}</span>
                                    </div>
                                `;
                            }});
                            resultsHtml += `
                    ...
                    resultsHtml += `
                        ...
                        <!-- Morphic Insights Section -->
                        <div class="analysis-section">
                            <h3>🧠 Morphic Resonance Insights</h3>
                            <div class="morphic-content">
                    `;

                    if (analysis.morphic_insights) {{
                        const morphic = analysis.morphic_insights;

                        // Add visualizations
                        if (morphic.visualizations) {{
                            Object.entries(morphic.visualizations).forEach(([key, path]) => {{
                                resultsHtml += `
                                    <div class="chart-container">
                                        <img src="/${{path}}" alt="${{key.replace('_', ' ')}}" class="analysis-chart" />
                                    </div>
                        ...
                        if (morphic.technical_details) {{
                            resultsHtml += `
                                <div class="technical-details">
                                    <h4>⚙️ Technical Implementation</h4>
                                    <div class="tech-grid">
                            `;
                            Object.entries(morphic.technical_details).forEach(([key, value]) => {{
                                resultsHtml += `
                                    <div class="tech-item">
                        ...
                        // Add insights
                        if (morphic.insights) {{
                            Object.entries(morphic.insights).forEach(([key, value]) => {{
                                if (typeof value === 'object') {{
                                    ...
                                    Object.entries(value).forEach(([subKey, subValue]) => {{
                                        resultsHtml += `<p><strong>${{subKey.replace('_', ' ')}}:</strong> ${{subValue}}</p>`;
                                    }});
                                    resultsHtml += `
                                            </div>
                                        </div>
                    ...
                    if (analysis.comparative_charts) {{
                        const comparative = analysis.comparative_charts;

                        // Add charts
                        if (comparative.charts) {{
                            Object.entries(comparative.charts).forEach(([key, path]) => {{
                                resultsHtml += `
                                    <div class="chart-container">
                                        <img src="/${{path}}" alt="${{key.replace('_', ' ')}}" class="analysis-chart" />
                                    </div>
                        ...
                        // Add insights
                        if (comparative.insights && comparative.insights.length > 0) {{
                            resultsHtml += `
                                <div class="insights-box">
                        ...
                        if (comparative.summary) {{
                            ...
                            `;
                            Object.entries(comparative.summary).forEach(([simType, summary]) => {{
                                resultsHtml += `
                                    <div class="performance-card ${{summary.performance_tier.toLowerCase()}}">
                                        <h5>${{simType.charAt(0).toUpperCase() + simType.slice(1)}}</h5>
                                        <p><strong>Performance:</strong> ${{summary.performance_tier}}</p>
                                        <p><strong>Best at:</strong> ${{summary.strongest_metric.replace('_', ' ')}}</p>
                                        <p><strong>Score:</strong> ${{(summary.overall_performance * 100).toFixed(1)}}%</p>
                                    </div>
                                `;
                            }});
                            resultsHtml += `
                    ...
                    resultsHtml += `
                        ...
                        <!-- Conway Factoids Section -->
                        <div class="analysis-section">
                            <h3>🎓 Conway's Game of Life Factoids</h3>
                            <div class="factoids-grid">
                    `;

                    if (analysis.conway_factoids && analysis.conway_factoids.length > 0) {{
                        analysis.conway_factoids.forEach(factoid => {{
                            resultsHtml += `
                                <div class="factoid-card">
                                    <h4>${{factoid.title}}</h4>
                                    <p>${{factoid.text}}</p>
                                    <div class="factoid-category">${{factoid.category}}</div>
                                </div>
                            `;
                        }});
                    }}
                    ...
                    resultsHtml += `
                        ...
                        <div class="permalink-section">
                            ...
                            <div class="permalink-box">
                                <input type="text" id="permalink-input" value="${{window.location.href}}" readonly>
                                <button onclick="copyPermalink()" class="btn btn-primary">📋 Copy Link</button>
                            </div>
                            <p>This permanent link provides access to your complete morphic resonance analysis.</p>
                        </div>
                    `;
